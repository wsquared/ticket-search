...
export class TicketRepository {
  ...
  getByDescription = async (term: string) => {
    return (await this.getBy())
      .filter(ticket => ticket.description.toLowerCase() === term.toLowerCase())
      .value();
  };
  ...
  getBySubject = async (subject: string) => {
    return (await this.getBy())
      .filter(ticket => ticket.subject.toLowerCase() === subject.toLowerCase())
      .value();
  };
  ...
  getByTag = async (tag: string) => {
    return (await this.getBy())
      .filter(
        ticket =>
          ticket.tags.map(t => t.toLowerCase()).indexOf(tag.toLowerCase()) >= 0
      )
      .value();
  };
