...
export class UserRepository {
  ...
  getByExternalId = async (id: string) => {
    return (await this.getBy()).filter(user => user.externalId === id).value();
  };
  ...
  getByActive = async (active: boolean) => {
    return (await this.getBy()).filter(user => user.active === active).value();
  };
  ...
  getByShared = async (shared: boolean) => {
    return (await this.getBy()).filter(user => user.shared === shared).value();
  };
  ...
  getBySignature = async (signature: string) => {
    return (await this.getBy())
      .filter(user => user.signature.toLowerCase() === signature.toLowerCase())
      .value();
  };
